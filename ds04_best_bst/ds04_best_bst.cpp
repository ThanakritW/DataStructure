...
void process(int left, int x, int right, CP::map_bst<int, int> &bst)
{
    // std::cout << left << ' ' << x << ' ' << right << '\n';
    bst[x] = x;
    if (x == left or x == right)
        return;
    if (x > left and bst.find(left + (x - left) / 2) == bst.end())
        process(left, left + (x - left) / 2, x, bst);
    if (x < right and bst.find(x + (right - x) / 2) == bst.end())
        process(x, x + (right - x) / 2, right, bst);
}
